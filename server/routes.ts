  ...
  app.post('/api/rpg/bands/from-audio', authMiddleware, async (req: any, res) => {
    try {
      ...
      try {
        ...
        console.log('Parsed band profile:', JSON.stringify(bandProfile, null, 2));
        
        // SAFETY FALLBACK: If AI didn't return a genre but user provided one, use it
        // (This should rarely happen with the improved prompt)
        if (!bandProfile.genre && userPreferences?.genrePreference) {
          console.warn(`⚠️ AI didn't return genre field - using user preference as fallback: ${userPreferences.genrePreference}`);
          bandProfile.genre = userPreferences.genrePreference;
        }
        
        // If still no genre, default to "Unknown"
        if (!bandProfile.genre) {
          console.warn('⚠️ No genre from AI or user - defaulting to "Unknown"');
          bandProfile.genre = 'Unknown';
        }
        
        // Validate the generated profile with detailed error messages
        const missingFields: string[] = [];
        if (!bandProfile.bandName) missingFields.push('bandName');
        if (!bandProfile.members || !Array.isArray(bandProfile.members) || bandProfile.members.length === 0) {
          missingFields.push('members (must be non-empty array)');
        }
        
        if (missingFields.length > 0) {
          console.error('AI response missing required fields:', missingFields);
          console.error('Received profile structure:', Object.keys(bandProfile));
          console.error('User preferences were:', userPreferences);
          throw new Error(`Invalid band profile generated by AI. Missing fields: ${missingFields.join(', ')}`);
        }
        
        console.log('✅ Band profile validation passed with genre:', bandProfile.genre);
