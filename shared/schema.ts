...
export const users = pgTable("users", {
  ...
  subscriptionPlan: varchar("subscription_plan")
    .default("free")
    .notNull(), // 'free', 'studio', 'creator', 'producer', 'mogul'
...
export const ImageEngine = z.enum([
  "dall-e-2",
  "dall-e-3",
  "flux",
  "midjourney",
...
export function isPlanFeatureEnabled(planType, feature) {
  return PLAN_FEATURES[planType][feature]
}

// Helper function to get allowed options for a plan
export function getAllowedOptions(planType, optionType) {
  const featureMap = {
  ...
  return PLAN_FEATURES[planType][featureMap[optionType]]
}
...
export const UNLIMITED_SERVICE_PLANS = {
...
export const userInventory = pgTable("user_inventory", {
  ...
  quantity: integer("quantity")
    .default(1)
    .notNull(), // For stackable items
  isActive: integer("is_active")
    .default(1)
    .notNull(), // 1 = equipped/active, 0 = owned but inactive
...
export const insertServiceRegistrySchema = createInsertSchema(
  serviceRegistry
).omit({
  totalAttempts: true,
  totalSuccesses: true,
