...
-   **KIE.ai Midjourney API**: For premium image generation, accessed via the Webshare static proxy. Generates 4 image variants per request with text-to-image mode only (reference images not supported via KIE.ai). Available to Studio+ users with two speed modes: Fast (~60-90s, 3 credits) and Turbo (~15-60s, 6 credits). Uses Midjourney v7 by default. Fast mode costs ~$0.04 API, Turbo costs 2x (~$0.08 API). Features comprehensive timeout + auto-refund system: Turbo times out after 120s, Fast after 180s, with automatic credit refunds for metered users (unlimited plans bypass refunds to prevent credit inflation). Real-time queue status UI shows estimated time, progress bar, and loading spinner during generation.
-   **ttapi.io Midjourney API**: Alternative Midjourney provider for A/B testing and comparison. Available to Studio+ users with same credit costs (Fast: 3 credits, Turbo: 6 credits). Supports three speed modes: Fast (~60-90s), Turbo (~15-60s), and Relax (slower, same cost as Fast). Same timeout + auto-refund system as KIE.ai. Displayed with green UI theme to distinguish from purple KIE.ai provider.
